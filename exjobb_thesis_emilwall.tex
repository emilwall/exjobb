...
\begin{table}
  \centering
    ...
    \end{tabular}
  \caption{Time in ms to run 287 tests with JsTestDriver in headless and non-headless browsers. Average for PhantomJS was 246 ms, whereas Safari ran the tests in 186 ms on average -- 32 \% faster. The other non-headless browsers lag behind at roughly half the speed of PhantomJS.}
  \label{tab:testruns}
\end{table}

The \gls{js} engine of PhantomJS was somewhat outdated compared to Safari (534.34 versus 537.71) and since the tests contained no dom manipulation that would trigger rendering activities in a non-headless browser, the Safari could perform well despite not being headless. Some efforts were made to run the tests in PhantomJS directly (using \url{https://github.com/jcarver989/phantom-jasmine}), to eliminate the overhead of the JsTestDriver integration, but either no tests were run or uninformative error messages were given, although the same test fixture worked in Safari (with a negligible difference in execution time compared to running the tests with JsTestDriver).
