...
The static typing of \gls{sml} means that in order to get good error reports from equality \glspl{matcher}, you either have to pass a toString function for the given type, define functions for each type: toEqualStr, toEqualInt, etc. or let the REPL (Read-Eval-Print-Loop) display the results. You could also define the \glspl{matcher} in separate namespaces (structures), as is done in smell-spec\footnote{\url{https://github.com/davidpdrsn/smell-spec/assertions.sml}}. Passing a toString function or letting the REPL display the results are probably the only ways to handle user defined datatypes. The downsides are that if a toString function must be passed, it makes the tests more repetitive and harder to read, and if the REPL is used the output can not as easily be written to a file or output as XML, JSON or any other specific format. It might be a good idea to use local definitions of \glspl{matcher} defined for a set of specs.
