...
One not so uncommon response is that testing of JavaScript probably is so uncommon because people programming in JavaScript often have a background as web graphic designers, without that much experience of automated testing. Another common (mis)conception is that JavaScript in general is not testable because it has too much to do with the front-end parts of an application.
