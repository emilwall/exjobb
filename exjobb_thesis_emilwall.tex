...
There are some desirable properties for unit tests, they should be fast, stable and without outer dependencies. Such dependencies can for instance be databases, network communication or a graphics library, and are avoided through stubbing and mocking (see section~\ref{subsec:mocking}). If the testing suite is not frequently brought into a state where all tests pass or it becomes to cumbersome to run the tests, there is a significant risk that developers will not do it as often \cite[question~2]{Rovegard}.
