...
Another consideration about failure messages is the order in which they are displayed. Preferably, they should be printed in the same order as the tests are executed, and the tests should be executed in the order that they are defined, because even though tests should be written so that the order in which they are executed does not matter it can be confusing for the user if the execution order is not clear.

Är det viktigt om man skriver to equal eller to be? Om båda finns som matchers så kan det vara viktigt, men då tillkommer å andra sidan ytterligare komplexitet i testramverket.
