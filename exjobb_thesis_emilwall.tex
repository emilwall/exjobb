...
There are indications that the overall quality of JavaScript code has improved over the last few years, much thanks to new application frameworks (as mentioned in section~\ref{sec:appframeworks}) but also as a consequence of new ways of working entering the JavaScript community. In connection with the increased popularity of the language and new capabilities thanks to the frameworks, people with background from backend-programming with tests, experienced with for example Ruby on Rails development, have influenced the way JavaScript code is written. \cite[questions~12-15]{Ahnve}
