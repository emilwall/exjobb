...
Two basic approaches to testing asynchronous code are to force the execution into a certain path by faking the asynchrony and to set up waiting conditions for when assertions should be executed. In the first alternative, the code is not executed asynchronously but you can test different scenarios and thereby get a good feel of how the code will behave when run asynchronously. In the second alternative, a timeout needs to be set for when the test should fail if the waiting condition is not met.

\subsection{DOM manipulation}

A key to successful testing of JavaScript with DOM manipulation is to separate the logic from the DOM manipulation as much as possible. One technique that was presented at the XP conference in Trondheim 2010\footnote{Agile Processes in Software Engineering and Extreme Programming 11th International Conference, XP 2010, Trondheim, Norway, June 1-4, 2010, Proceedings} was to use different layers of code and update the view as a separate task. This allows for test driven development to be carried out much more effortlessly. \cite[question~4]{Ahnve}
