...
Much time was spent programming, including working through the projects of \cite[part~III]{Tddjs}, adding tests to an existing \gls{js} application (see section \ref{subsec:asteriods}), developing an \gls{sml} testing framework (see section \ref{subsec:describesml}) and using the developed framework in practise to solve programming assignments in a \gls{mooc}, namely the Coursera Programming Languages course (see section~\ref{subsec:tddmooc}).
...
\begin{table}
  \centering
    ...
    \end{tabular}
  \caption{Method choice depending on type of bugs}
  \label{tab:fvt2}
\end{table}

Kan formell verifiering ha liknande effekter på modularitet och utformning av gränssnitt som testdriven utveckling? Modularitet ja, eftersom det är enklare att konstruera bevis för korrekthet av isolerade enheter. FV är däremot oftast olämpligt när det kommer till att testa gränssnitt.
...
A real world example of \gls{sml} \gls{testing} is the Coursera \gls{mooc} Programming Languages\footnote{\url{https://www.coursera.org/course/proglang}}. In that course (and others, such as the Principles of Reactive Programming course), students are encouraged to test their code and required to hand in their tests together with their solutions. The handed in tests are not graded, and hardly read by the course administration to any large extent, but the requirement still has an effect on the amount of testing done by the students.
