...
\section{Stenmark Interview Summary}
...
In order to understand the problems with testability that can be seen in JavaScript applications, it is important to understand how many were first introduced to the language. When jQuery was released in 2006, people used it for animations and evolved a coding style based on handlers with DOM manipulation through selectors and asynchronous callbacks. This caused even experienced developers to fail to apply principles for writing testable and maintainable code, since tutorials were written in a quick-and-dirty fashion. Indeed, the slogan for jQuery is ``write less, do more''. \cite[question~10]{Stenmark}
