...
There is only one major testing framework available for SML, namely QCheck\footnote{\url{https://github.com/league/qcheck}}. A few others exist but have not gained any traction and are relatively small: three of them are called
...
stupid\footnote{\url{https://github.com/teebrz/stupid}}. Most of them are small, smell-spec is the only one with BDD syntax and stupid is unique in offering watch capabilities so that the tests are run when files change.
