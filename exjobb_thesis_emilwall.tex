...
In section \ref{sec:patternsexamples}, it is mentioned that examples are often simple and seldom provide a full picture. In contrast, the problem domain of this thesis is to focus on how to test the behaviour of JS that manipulates DOM elements (see \ref{sec:dom}), fetches data using asynchronous calls, validates forms, communicates through APIs or manipulates the appearance of a web page. We also look at when and why you should test your JS.
...
\item How JS is typically conceived as a language and how it is used (section~\ref{sec:interviewees})
\item Economic viability and risk awareness (section~\ref{sec:projectrisks})
\end{itemize}
...
\item Usability of testing tools and frameworks (sections \ref{sec:angularjasminekarma}, \ref{sec:jstestdriverintegration} and \ref{sec:jstestdriver})
...
\label{sec:stateofevents}

It's possible that the overall quality of JS code has improved over the last few years, partly as a consequence of testing methodologies entering the JS community. One might argue that the new application frameworks has brought not only new capabilities (as mentioned in section~\ref{sec:frameworks}) but also brought people with background from backend-programming with tests, experienced with for example Ruby on Rails development, to JS. In fact, most of the new frameworks have been developed by people with such background. This has influenced the way JS code is written and given birth to a novel testing culture. \cite[questions~12-15]{Ahnve}

\subsection{Individual Motivation and Concerns}
\label{sec:motivationconcerns}
...
\label{sec:jstestdriverintegration}
