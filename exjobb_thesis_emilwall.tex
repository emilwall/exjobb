...
\Gls{dom} manipulating code can be tested if necessary, but there are many implementation differences depending on which browser or testing environment the tests are run in which means that the tests have to run in different browser implementations to avoid a false sense of security (on the other hand, this is one of the reasons why you might WANT to test the \gls{dom} manipulations) and, as both Stenmark and Wendt mentioned in their emails (see Appendix), that it can be hard to keep test fixtures fast and up to date \cite{Stenmark}\cite{Wendt}.
