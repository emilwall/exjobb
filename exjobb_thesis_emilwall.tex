...
First off, people do test their \gls{sml} code, but not always, and very seldom using the same style and convention when doing it. There is a need for a simple but good testing framework, QCheck/SML has great benefits but is too hard to use for the average programmer or in scenarios where the process of writing tests need to be quick. Other frameworks, including DescribeSML, are too immature to date, so a collaboration would be beneficial for all.
