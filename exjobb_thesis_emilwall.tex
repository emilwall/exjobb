...
While writing this thesis, much time was spent programming: working through the projects in part III of \cite{Tddjs}, adding tests to an existing gls{js} application (see section \ref{subsec:asteriods}), developing an gls{sml} testing framework (see section \ref{subsec:describesml}) and using that framework in practise to solve programming assignments in a MOOC course.
...
Sinon.JS\footnote{\url{http://sinonjs.org/}}. Apart from the MOOC programming assignments, all code is publicly available on my Github account \emph{emilwall}, together with the \LaTeX~code for this report.
...
By the time I found out about existing SML testing frameworks (apart from QCheck) I had already started to write my own. I wanted to use my insights from using JS testing frameworks such as Jasmine and Mocha to write a BDD framework which I would use in the Coursera MOOC course Programming Languages. % Add references to the course, MOOC explanation (perhaps in glossary?)
