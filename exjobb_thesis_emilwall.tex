...
Sometimes during the process adding tests to the asteroids application it happened that a dependency was accidentally overlooked and not properly stubbed. Detecting this sort of test smell was non-trivial, because there was no automated way of ensuring that each test executed in isolation, accessing only the part of the application that it was supposed to test. Attempting to stub a function which is not defined produces an exception in sinon.JS, in order to prevent you from making typing errors, so manually ensuring that just the appropriated files were loaded was not an option. This could be circumvented by manual stubbing, but abandoning sinon.JS for that reason felt inconvenient, especially considering that the separation from the code that should not be executed by the tests had to be done manually, so instead I opted towards simply being more careful not to miss any calls that should be stubbed. A semi-automated approach might be possible, using coverage reports to inspect that only the desired code is executed when a certain test is run, but this was not investigated.
