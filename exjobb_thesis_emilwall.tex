...
Looking at the source code of SML implementations such as SML/NJ and Moscow ML, you quickly realise that there are tests for large portions of the code, but there is no standard way of writing the tests. Each module typically has its own scripts for testing, that is invoked differently and produces different output. If you search the web for ``Standard ML testing framework'', you get zero results. Dig deeper, and you'll find that there is a large testing framework out there, which goes under the name QCheck/SML, and quite a few small testing framework projects hosted on GitHub (see Appendix). It seems that SML developers prefer writing their own testing framework rather than using an existing one.

As a programmer, you commonly strive to do the simplest thing possible within a certain set of constraints, and to reuse as much as possible in any given situation, provided that the reuse candidate meets certain quality standards and is easy enough to adapt to your needs. XP (Extreme Programming) principles such as YAGNI (You Ain't Gonna Need It) are incorporated in our way of thinking, so the natural thing when starting to write tests in a project is to mimic how other tests in the project or previous projects have been written. If there are no such conventions to rely on and you can not find anything on the web, or you don't like what you find, you start writing your own utilities for testing. Why is it that so many end up doing this?
