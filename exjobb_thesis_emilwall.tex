...
\subsection{(JS0,ML1)QCheck/SML and Formal Verification}

QuickCheck, the Haskell predecessor of QCheck/SML is faced with a similar challenge since Haskell is also statically typed. It is resolved by using the \texttt{where} keyword to specify the types of properties\footnote{\url{http://www.cse.chalmers.se/~rjmh/QuickCheck/manual\_body.html\#4}}, which is pretty elegant. % TODO gör om till riktig referens, helst annan källa

Sometimes there are different driving forces for testing different languages (such as \gls{js} and \gls{sml}), simply because how they are used by people with different background and motives, and in such diverse applications. Why is it that the only truly popular testing framework for \gls{sml} is based on generating test cases for properties rather than expectations/assertion based testing? Possibly because it was developed by academic people, or because mathematicians are drawn to the language and are inclined to think in terms of properties and how they can be falsified rather than state or expected output. Another possible reason is that testing for expected output is so easy even without the use of a testing framework, whereas QCheck/SML solves problems that are not as easily solved ad hoc.

There is a possibility that people simply do not find it worth their time to learn simple testing frameworks just to get readable tests and good failure reports, but instead are content with simply declaring tests as variables that evaluate to true or false, or similar. The consequences of doing this, however, are likely to be increased amount of debugging and risk of missing a failed test or spending a lot of time scrolling through the result. Sine \gls{sml} has scripting capabilities, it is pretty easy to write and run tests without a testing framework compared languages that are more verbose and less often used in an interpreted setting, such as Java. Adding a testing framework becomes a bigger deal in \gls{sml} for this reason.

Formal verification is related to property based testing, but more rigorous. Formal verification is performed by manually constructing mathematical proof of correctness, theorem proving, or deciding whether a model satisfies a given property (known as model or property checking). Model checking is most easily automated, either by specialised software such as CADP\footnote{\url{http://cadp.inria.fr}}, UPPAAL\footnote{\url{http://www.uppaal.org}} and many more\footnote{\url{http://en.wikipedia.org/wiki/List_of_model_checking_tools}} (a couple of them written in \gls{sml}) or by using a constraint solver (see \url{http://www.cs.utah.edu/formal\_verification/pdf/ligd\_thesis.pdf}. Kan formell verifiering ha liknande effekter på modularitet och utformning av gränssnitt som testdriven utveckling? % Add Formal verification to glossary. Make a real reference of thesis.

Hur mycket tid tar en utförlig formell verifiering jämfört med testning?

Vilken kompetens krävs for formell verifiering?
...
The static typing of \gls{sml} means that in order to get good error reports from equality \glspl{matcher}, you either have to pass a toString function for the given type, define functions for each type: toEqualStr, toEqualInt, etc. or let the REPL (Read-Eval-Print-Loop) display the results. You could also define the \glspl{matcher} in separate namespaces (structures), as is done in smell-spec\footnote{\url{https://github.com/davidpdrsn/smell-spec/assertions.sml}}.

Passing a toString function or letting the REPL display the results of an equality \gls{matcher} are probably the only ways to print informative failure reports for user defined datatypes. The downsides are that if a toString function must be passed to each \gls{matcher}, it imposes verbosity and make the tests harder to read, and if the REPL is used the output can not as easily be written to a file or output as XML or other formats. It could be a good idea to use local definitions of \glspl{matcher} defined for a set of specs, for example:
...
However, this leads to increased indentation level and verbosity, and limits any local (possibly shadowing as in the example) \gls{matcher} definition to be used only for the specified type, which is not always very convenient. The matchers that are otherwise hard coded in the framework due to the static typing are toEqual, toEqualStr, toEqualInt, etc. The worst part however is defining matchers for lists and tuples because you might end up needing things such as toContainStr, toEqualIntList or toEqualIntTuple5 (for instance, defined for int 5-tuples only). The real issue is getting informative and practical failure messages for all the types, user defined datatypes included, and to ensure that failure messages are output in a way so that you do not miss them by mistake.
