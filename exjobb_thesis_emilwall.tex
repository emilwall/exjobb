...
\title{Rationales and Approaches for Automated Testing of JavaScript and ML}
\author{Emil Wall}
...
The scope of this thesis has been to look mainly at automated testing of ML and \emph{client side} JavaScript. This meant that the impact of frameworks specialised for server side code such as vows\footnote{\url{http://vowsjs.org/}} and cucumis\footnote{\url{https://github.com/noblesamurai/cucumis}} have not been considered. Testing client side code is by no means more important than the server side, but often harder and less obvious how to generalise from testing principles in other languages.

Frameworks that are no longer maintained such as JsUnit\footnote{\url{https://github.com/pivotal/jsunit}} and JSpec\footnote{\url{https://github.com/liblime/jspec}} have deliberately been left out of consideration. Others have been left out because of a smaller user base or lack of unique functionality; among these we find TestSwarm, YUI Yeti and RhinoUnit. These are useful tools but since they are so many it would not be realistic to include them all.

Manual testing is not covered to any significant extent, since there are enough issues to deal with in automated testing. Naturally, there are many situations where manual testing is required, but in this thesis when we say testing we typically refer to automated testing.
