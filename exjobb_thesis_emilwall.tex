...
The unit tests for the asteroids application had an average execution time of roughly 1 ms per test, so there was little to gain from using a headless browser. However, it was done anyway for the sake of research, using \url{https://github.com/larrymyers/js-test-driver-phantomjs}. The results were somewhat surprising -- the execution time increased by an average of 33 \% (see table~\ref{tab:testruns}), rather than decreasing as would be expected. The experiment was later (2013-12-10) repeated but with Chrome (31.0.1650.63) also included among the browsers. Safari still outran PhantomJS, which in turn was roughly twice as fast as Chrome (except for a single outlier).

\begin{table}
  \centering
    \begin{tabular}{ c | c c}
    Test run & PhantomJS & Safari \\ \hline
    1 & 299 & 225 \\
    2 & 348 & 323 \\
    3 & 261 & 238 \\
    4 & 299 & 204 \\
    ...
    8 & 363 & 271 \\
    \end{tabular}
  \caption{Time in ms to run 287 tests with JsTestDriver in headless and non-headless browsers. Average for PhantomJS was 311 ms, whereas Safari ran the tests in 234 ms on average -- 33 \% faster.}
  \label{tab:testruns}
\end{table}

Probably the \gls{js} engine of PhantomJS (1.9.2, installed using npm) was outdated compared to the non-headless browser (Safari 7.0, 537.71 for OS X 10.9) and since the tests contained no dom manipulation that would trigger rendering activities in a non-headless browser, the headless browser was unable to provide its usual benefits. Some efforts were made to run the tests in PhantomJS directly (using \url{https://github.com/jcarver989/phantom-jasmine}), to eliminate the overhead of the JsTestDriver integration, but either no tests were run or uninformative error messages were given, although the same test fixture worked in the non-headless browser (with a negligible difference in execution time compared to running the tests with JsTestDriver).
